...
public:
    ...
    /// Create a not-yet-connected placeholder (broken() == true).
    /// Useful for tests and for lazily populating a connection pool.
    static Connection make_unconnected(const ConnectionOptions &opts) {
        return Connection(opts, Dummy{});
    }
    ...
    /// dropping the TCP connection. Updates stored options on success.
    /// On failure the connection is left broken so the pool can reconnect lazily.
