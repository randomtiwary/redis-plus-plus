...
public:
    ...
    /// Register a synchronous connection pool so it receives password updates
    /// and gradual reauth. Pools are held weakly and cleaned up automatically.
    void register_pool(const std::shared_ptr<ConnectionPool> &pool);

#ifdef REDIS_PLUS_PLUS_HAS_ASYNC
    /// Register an asynchronous connection pool (same rotation semantics as
    /// `register_pool`, using inline async AUTH / gradual soft-drop).
    void register_async_pool(const std::shared_ptr<AsyncConnectionPool> &pool);
#endif

    /// Number of successful token fetches (background + `refresh_now`).
    std::size_t refresh_count() const;

    /// Number of currently registered (still-alive) pools (sync + async).
    /// Intended for tests.
...
#ifdef REDIS_PLUS_PLUS_HAS_ASYNC
    std::vector<std::shared_ptr<AsyncConnectionPool>> _alive_async_pools();
#endif
...
#ifdef REDIS_PLUS_PLUS_HAS_ASYNC
    std::vector<std::weak_ptr<AsyncConnectionPool>> _async_pools;
#endif
